...
export default class TableColumnComponentsPage extends AlertPage {
  title: ElementFinder = element(by.id('table-column-heading'));
...
export class TableColumnDeleteDialog {
  dialog: ElementFinder = element(by.id('removeEntity'));
  title: ElementFinder = element(by.id('dataelyAppV3App.tableColumn.delete.question'));
  confirmButton: ElementFinder = element(by.id('jhi-confirm-delete-tableColumn'));
}
