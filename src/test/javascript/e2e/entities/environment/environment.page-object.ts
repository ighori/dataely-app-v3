...
export default class EnvironmentComponentsPage extends AlertPage {
  title: ElementFinder = element(by.id('environment-heading'));
...
export class EnvironmentDeleteDialog {
  dialog: ElementFinder = element(by.id('removeEntity'));
  title: ElementFinder = element(by.id('dataelyAppV3App.environment.delete.question'));
  confirmButton: ElementFinder = element(by.id('jhi-confirm-delete-environment'));
}
