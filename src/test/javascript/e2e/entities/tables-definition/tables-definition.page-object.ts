...
export default class TablesDefinitionComponentsPage extends AlertPage {
  title: ElementFinder = element(by.id('tables-definition-heading'));
...
export class TablesDefinitionDeleteDialog {
  dialog: ElementFinder = element(by.id('removeEntity'));
  title: ElementFinder = element(by.id('dataelyAppV3App.tablesDefinition.delete.question'));
  confirmButton: ElementFinder = element(by.id('jhi-confirm-delete-tablesDefinition'));
}
