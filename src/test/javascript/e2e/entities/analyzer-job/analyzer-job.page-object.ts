...
export default class AnalyzerJobComponentsPage extends AlertPage {
  title: ElementFinder = element(by.id('analyzer-job-heading'));
...
export class AnalyzerJobDeleteDialog {
  dialog: ElementFinder = element(by.id('removeEntity'));
  title: ElementFinder = element(by.id('dataelyAppV3App.analyzerJob.delete.question'));
  confirmButton: ElementFinder = element(by.id('jhi-confirm-delete-analyzerJob'));
}
