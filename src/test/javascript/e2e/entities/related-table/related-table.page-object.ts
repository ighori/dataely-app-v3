...
export default class RelatedTableComponentsPage extends AlertPage {
  title: ElementFinder = element(by.id('related-table-heading'));
...
export class RelatedTableDeleteDialog {
  dialog: ElementFinder = element(by.id('removeEntity'));
  title: ElementFinder = element(by.id('dataelyAppV3App.relatedTable.delete.question'));
  confirmButton: ElementFinder = element(by.id('jhi-confirm-delete-relatedTable'));
}
