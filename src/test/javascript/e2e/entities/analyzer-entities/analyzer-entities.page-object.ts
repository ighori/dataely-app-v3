...
export default class AnalyzerEntitiesComponentsPage extends AlertPage {
  title: ElementFinder = element(by.id('analyzer-entities-heading'));
...
export class AnalyzerEntitiesDeleteDialog {
  dialog: ElementFinder = element(by.id('removeEntity'));
  title: ElementFinder = element(by.id('dataelyAppV3App.analyzerEntities.delete.question'));
  confirmButton: ElementFinder = element(by.id('jhi-confirm-delete-analyzerEntities'));
}
