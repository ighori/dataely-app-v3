...
export default class FileConfigComponentsPage extends AlertPage {
  title: ElementFinder = element(by.id('file-config-heading'));
...
export class FileConfigDeleteDialog {
  dialog: ElementFinder = element(by.id('removeEntity'));
  title: ElementFinder = element(by.id('dataelyAppV3App.fileConfig.delete.question'));
  confirmButton: ElementFinder = element(by.id('jhi-confirm-delete-fileConfig'));
}
