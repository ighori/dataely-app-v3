...
export default class DsSchemaComponentsPage extends AlertPage {
  title: ElementFinder = element(by.id('ds-schema-heading'));
...
export class DsSchemaDeleteDialog {
  dialog: ElementFinder = element(by.id('removeEntity'));
  title: ElementFinder = element(by.id('dataelyAppV3App.dsSchema.delete.question'));
  confirmButton: ElementFinder = element(by.id('jhi-confirm-delete-dsSchema'));
}
