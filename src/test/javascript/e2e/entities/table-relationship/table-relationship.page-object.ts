...
export default class TableRelationshipComponentsPage extends AlertPage {
  title: ElementFinder = element(by.id('table-relationship-heading'));
...
export class TableRelationshipDeleteDialog {
  dialog: ElementFinder = element(by.id('removeEntity'));
  title: ElementFinder = element(by.id('dataelyAppV3App.tableRelationship.delete.question'));
  confirmButton: ElementFinder = element(by.id('jhi-confirm-delete-tableRelationship'));
}
